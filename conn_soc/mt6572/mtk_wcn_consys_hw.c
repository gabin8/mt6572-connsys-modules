...
set_pin_exit:
	...
	if (node) {
		...
		if (!pmic_regmap || IS_ERR(pmic_regmap))
			goto set_pmic_wrap_exit;
	} else {
...
set_pmic_wrap_exit:
	...
	/* mt6572: CONN pins are dedicated balls, no pinctrl needed */
	consys_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(consys_pinctrl)) {
		WMT_PLAT_WARN_FUNC("no consys pinctrl (ok on mt6572), ret=%ld\n",
				   PTR_ERR(consys_pinctrl));
		consys_pinctrl = NULL;
	}
	return 0;
}
...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on, UINT32 co_clock_type)
{
	...
	if (on) {
		...
		if ((0 == retry) || (0 == consysHwChipId))
			WMT_PLAT_WARN_FUNC("chipId poll timed out (0x%08x) - known non-fatal on mt6572, FW download boots the core\n", consysHwChipId);

		/*13.force AP2CONN_OSC_EN (26M grant) while the AP is awake, as
		 * downstream does at init: without it the chip cannot be woken
		 * from PSM sleep (the BTIF WAK pulse needs the granted 26M). */
		CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AP2CONN_OSC_EN_OFFSET,
				 CONSYS_REG_READ(conn_reg.topckgen_base +
						 CONSYS_AP2CONN_OSC_EN_OFFSET) | CONSYS_AP2CONN_OSC_EN_BIT);
		WMT_PLAT_INFO_FUNC("AP2CONN_OSC_EN forced on, reg(0x%08x)\n",
...
INT32 mtk_wcn_consys_hw_restore(struct device *device)
{
	...
	gConEmiPhyBase = CONSYS_EMI_AP_PHY_BASE - CONSYS_EMI_AP_PHY_OFFSET;
	if (gConEmiPhyBase) {
...
/* mt6572: EMI diagnostics window is at a fixed AP-phys address that the
 * FW-side base (0xf0080000) falls through to; there is no remap register
 * on this chip and the region lies outside kernel-managed RAM. */
...
INT32 mtk_wcn_consys_hw_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6572-consys");
	if (node) {
	...
	emi_mpu_set_region_protection(gConEmiPhyBase + SZ_1M/2,
		...
		(void)addrPhy; /* no CONSYS EMI remap register on mt6572 */
