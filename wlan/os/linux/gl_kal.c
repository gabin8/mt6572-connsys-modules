...
static const struct firmware *prWlanFw;
...
{
	struct device *prDev;
	char aucChipInfo[16] = { 0 };
	char aucFwName[48];

	ASSERT(prGlueInfo);
	prDev = prGlueInfo->rHifInfo.Dev;

	glGetChipInfo(prGlueInfo, (UINT_8 *) aucChipInfo);
	snprintf(aucFwName, sizeof(aucFwName), "%s_%s", CFG_FW_FILENAME, aucChipInfo);

	/* the stock MT6572 driver has /etc/firmware/WIFI_RAM_CODE_MT6582
	 * compiled in — that variant MUST win; the plain image is for a
	 * different chip build and leaves the fw PC parked in ROM */
	if (request_firmware(&prWlanFw, CFG_FW_FILENAME "_MT6582", prDev) == 0)
		goto done;
	if (request_firmware(&prWlanFw, aucFwName, prDev) == 0)
		goto done;
	if (request_firmware(&prWlanFw, CFG_FW_FILENAME, prDev) == 0)
		goto done;
	...
	DBGLOG(INIT, INFO, "Open FW image done, %u bytes\n", (UINT_32) prWlanFw->size);
...
WLAN_STATUS kalFirmwareClose(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	release_firmware(prWlanFw);	/* NULL-safe */
	prWlanFw = NULL;
...
WLAN_STATUS kalFirmwareLoad(IN P_GLUE_INFO_T prGlueInfo, OUT PVOID prBuf, IN UINT_32 u4Offset, OUT PUINT_32 pu4Size)
{
	...
	if (!prWlanFw || u4Offset > prWlanFw->size)
		return WLAN_STATUS_FAILURE;

	u4Copy = min_t(UINT_32, *pu4Size, prWlanFw->size - u4Offset);
	kalMemCopy(prBuf, prWlanFw->data + u4Offset, u4Copy);
	*pu4Size = u4Copy;
...
WLAN_STATUS kalRxIndicatePkts(IN P_GLUE_INFO_T prGlueInfo, IN PVOID apvPkts[], IN UINT_8 ucPktNum)
{
	...
	for (ucIdx = 0; ucIdx < ucPktNum; ucIdx++) {
		...
		netif_rx(prSkb);	/* safe in any context since 5.18 */
